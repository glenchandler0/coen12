...
static int getDepth(struct tree *np)
{
    return np == NULL ? -1 : 1 + getDepth(getParent(np));
}


void pack(char *infile, char *outfile, struct tree *leaves[END + 1])
{
    unsigned char byte;
    ...
    printf("INTERPRETED ROOT SIZE: %d, %lld\n", getData(root), buf.st_size);//--
    if (getData(root) != buf.st_size) {
    //DELETE THIS LINE!!
    ...
    for (c = 0; c <= END; c ++) {
	if (leaves[c] != NULL) {
	    length[c] = getDepth(leaves[c]);
	    sum += length[c] * getData(leaves[c]);
